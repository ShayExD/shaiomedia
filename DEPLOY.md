...
**עובד.** Resend שולח, `shaiomedia.com` מאומת.
...
`LEAD_TO` תומך בכמה כתובות מופרדות בפסיק, אם תרצה עותק לכתובת נוספת.
`reply_to` נקבע לכתובת של הפונה, אז "השב" פותח מייל אליו ישירות.

### מלכודת שעלתה ביוקר, שלא תחזור

שדה המלכודת נגד בוטים נקרא פעם `company_website`. **מילוי אוטומטי של הדפדפן
מילא אותו**, השרת סיווג את הפונה כבוט, החזיר "הצלחה" ומחק את הליד. שני לידים
אמיתיים אבדו ככה.

שני תיקונים, ושניהם חשובים:

1. השדה נקרא עכשיו בשם שאף מנגנון מילוי לא מזהה, והוא `readonly` כך שהדפדפן
   לא יכול למלא אותו כלל. סקריפט שמציב ערך עדיין נתפס.
2. **מלכודת שנתפסת כבר לא מוחקת.** המייל נשלח עם `[ייתכן ספאם]` בנושא, ורק
   ה-CRM מדלג. המחיר של טעות לכיוון אחד הוא רעש, ולכיוון השני הוא לקוח אבוד.

### אימות שכדאי לחזור עליו אחרי כל שינוי בטופס

```bash
curl -s -X POST -H 'content-type: application/json' \
  -H 'origin: https://service.shaiomedia.com' \
  -d '{"name":"בדיקה","email":"you@example.com","phone":"0542192619","service":"גגות"}' \
  https://service.shaiomedia.com/api/lead
```

תשובה `{"ok":true}` פירושה שהמייל יצא בפועל.
